...
auto taskscheduler::get_jobs( ) -> std::vector<std::uintptr_t>
{
    ...
    auto current_job = *reinterpret_cast<std::uintptr_t**>(roblox::functions::get_tasksched( ) + roblox::offsets::JobListStart);
    auto end_job = *reinterpret_cast<std::uintptr_t**>(roblox::functions::get_tasksched( ) + roblox::offsets::JobListEnd);
    
    while (current_job < end_job) {
        job_list.push_back(*current_job);
        current_job += (sizeof(std::uintptr_t) == 8 ? 2 : 2); // In 64-bit, skip 16 bytes (2 pointers), wait.
        // Actually, the job list is usually an array of pointers.
        // On 64-bit, each pointer is 8 bytes.
    }
...
auto taskscheduler::log_jobs( ) -> void
{
    ...
    {
        auto job_name = *reinterpret_cast<const char**>(job + (sizeof(std::uintptr_t) == 8 ? 144 : 128));

        if ( !(*(std::uint8_t*)(job + (sizeof(std::uintptr_t) == 8 ? 136 : 120)) << 31) ) {
            job_name = (const char*)(job + (sizeof(std::uintptr_t) == 8 ? 137 : 121));
        }
...
{
    return *reinterpret_cast<uintptr_t*>(get_job_by_name("WaitingHybridScriptsJob") + roblox::offsets::Whsj_ScriptCtxt);
}
