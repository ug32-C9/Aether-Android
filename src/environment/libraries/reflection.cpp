...
static int cloneref(lua_State* ls)
{
    LOGD(" LuauEnvCall -> cloneref - CallingThread -> %p", ls);
    luaL_checktype(ls, 1, LUA_TUSERDATA);
    
    const auto inst = *reinterpret_cast<std::uintptr_t*>(lua_touserdata(ls, 1));
    roblox::functions::rlua_pushinstance(ls, inst);
    return 1;
}

static int compareinstances(lua_State* ls)
{
    LOGD(" LuauEnvCall -> compareinstances - CallingThread -> %p", ls);
    luaL_checktype(ls, 1, LUA_TUSERDATA);
    luaL_checktype(ls, 2, LUA_TUSERDATA);
    
    const auto inst1 = *reinterpret_cast<std::uintptr_t*>(lua_touserdata(ls, 1));
    const auto inst2 = *reinterpret_cast<std::uintptr_t*>(lua_touserdata(ls, 2));
    
    lua_pushboolean(ls, inst1 == inst2);
    return 1;
}

static int getscripts(lua_State* ls)
{
    LOGD(" LuauEnvCall -> getscripts - CallingThread -> %p", ls);
...
static int getmodulescripts(lua_State* ls)
{
    LOGD(" LuauEnvCall -> getmodulescripts - CallingThread -> %p", ls);
    ...
    if (lua_istable(ls, -1))
    {
        ...
        while (lua_next(ls, -2))
        {
            const auto inst_ptr = *reinterpret_cast<std::uintptr_t*>(lua_touserdata(ls, -1));
            auto inst = reflection::RbxInstance{ inst_ptr };
            if (inst.GetClassName() == "ModuleScript")
            {
                lua_pushvalue(ls, -1);
                lua_rawseti(ls, -5, idx++);
            }
...
static int gethiddenproperties(lua_State* ls)
{
    // For now, same as getproperties but with a different name
    return getproperties(ls);
}

static int getcallbackvalue(lua_State* ls)
{
    LOGD(" LuauEnvCall -> getcallbackvalue - CallingThread -> %p", ls);
    luaL_checktype(ls, 1, LUA_TUSERDATA);
    luaL_checktype(ls, 2, LUA_TSTRING);
    
    // Stub implementation: just try to get the field normally
