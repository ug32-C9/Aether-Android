...
static std::mutex startedJobsMutex;

void roblox::hooks::clear_jobs()
{
    std::lock_guard<std::mutex> lock(startedJobsMutex);
    startedJobs.clear();
}
...
static int jobstart_hook( std::uintptr_t job )
{
    ...
    // Updated offsets for 64-bit std::string (libc++)
...
void roblox::hooks::init( )
{
    ...
    const auto RobloxContextSystem_CapabilityErr = utils::memory::rebase( "libroblox.so", 0x38BDC04 );
