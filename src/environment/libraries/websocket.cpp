...
public:
    ...
    static int websocket_send(lua_State* ls)
    {
        ...
        std::string data = luaL_checkstring(ls, 2);
        
        exploit_websocket* webSocket = reinterpret_cast<exploit_websocket*>(lua_touserdata(ls, lua_upvalueindex(1)));
    ...
    int handleIndex(lua_State* ls)
    {
        ...
        if (idx == "OnMessage") {
            ...
            lua_pushvalue(ls, 1); // self
            lua_pushcclosure(ls, websocket_send, "WebSocket.Send", 1);
            return 1;
        }
        else if (idx == "Close") {
            lua_pushvalue(ls, 1); // self
            lua_pushcclosure(ls, websocket_close, "WebSocket.Close", 1);
            return 1;
        }
        return 0;
    };
...
static int websocket_index_handler(lua_State* ls)
{
    exploit_websocket* webSocket = reinterpret_cast<exploit_websocket*>(lua_touserdata(ls, lua_upvalueindex(1)));
    return webSocket->handleIndex(ls);
}
...
static int websocket_connect(lua_State* ls)
{
    ...
    *webSocket = exploit_websocket{ };
    ...
    lua_pushvalue(ls, -2);
    lua_pushcclosure(ls, websocket_index_handler, "__index", 1);
